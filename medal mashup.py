...
def getAllMedals(gamertag):
    medals = pd.DataFrame(
        lib.halo.infinite[version].metadata.multiplayer.medals()['data'])
    medals['image_urls'] = medals['image_urls'].apply(lambda x: x['large'])
    medals['count'] = 0

    result = lib.halo.infinite[version].stats.players['service-record'].multiplayer.matchmade({
        'gamertag': gamertag,
        'season': 1
    })
    medal_count = pd.DataFrame(
        result['data']['records']['pvp']['core']['breakdowns']['medals'])
    for i in range(len(medal_count)):
        medals.loc[medals['id'] == medal_count['id']
                   [i], 'count'] = medal_count['count'][i]
